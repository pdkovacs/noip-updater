...
const checker: MyPublicIpChecker = createMyPublicIpChecker(config.data.checker.impl, config.data.checker.initialIp);
const updater: NoIpUpdater = new NoIpUpdater(config.data.updater.auth);
const ipCheckRetrier: RetryTimer = new RetryTimer("ip-check",
...
const updateFirstHost: (hostnameList: Immutable.List<string>) => Promise<boolean> =
    hostnameList => {
        return updater.update(hostnameList.first()).then(result => {
            ...
            const nextList = hostnameList.shift();
            if (nextList.size > 0) {
